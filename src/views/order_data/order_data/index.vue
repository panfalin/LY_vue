...
function getList() {
  ...
  listOrder_data(queryParams.value).then(response => {
    ...
    totalAmount.value = order_dataList.value.reduce((total, item) => total + item.orderAmount, 0)
    totalPayment.value = order_dataList.value.reduce((total, item) => total + (item.tax + item.successfulRefundAmount), 0)
